    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto text-left">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredItems.length === 0 ? (
          <div className="col-span-full p-8 rounded-2xl bg-white border border-[#E2E8F0] text-center space-y-4 shadow-sm">
            <div className="w-12 h-12 rounded-2xl bg-blue-50 border border-blue-200/60 text-[#2563EB] mx-auto flex items-center justify-center shadow-xs">
              <Database className="w-6 h-6" />
            </div>
            <div className="max-w-md mx-auto space-y-1">
              <h3 className="text-base font-bold font-mono text-[#0F172A]">
                {evidenceItems.length === 0 ? 'Your Evidence Vault is Ready' : 'No matching records found'}
              </h3>
              <p className="text-xs text-[#64748B]">
                {evidenceItems.length === 0
                  ? 'Start your clean portfolio by depositing your first project codebase or hackathon award.'
            ...
            {evidenceItems.length === 0 ? (
              <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
                <button
                  onClick={onOpenAddProject}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#111827] hover:bg-black text-xs font-mono font-semibold text-white shadow-sm transition-all"
                >
                  <Plus className="w-4 h-4" />
                  <span>Deposit First Project</span>
                </button>
                <button
                  onClick={onOpenAddHackathon}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white hover:bg-[#F8FAFC] border border-[#E2E8F0] text-xs font-mono font-semibold text-[#0F172A] shadow-xs transition-all"
                >
                  <Trophy className="w-4 h-4 text-amber-600" />
                  <span>Add Hackathon Award</span>
                </button>
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-xs font-mono text-slate-700 font-semibold"
              >
                Reset Filter
              </button>
            )}
